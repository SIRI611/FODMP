...
class DPTransformerMetaWorld(BaseWorkspace):
    """
    Diffusion Policy Transformer workspace for MetaWorld environments.
    
    This workspace implements a diffusion policy using Transformer-based observation encoding
    for MetaWorld robotic manipulation tasks. It extends the base workspace with
    diffusion policy specific training and evaluation logic using transformer architectures.
    """
    ...
    def setup_agent(self, device: str = "cuda") -> None:
        """Initialize the diffusion policy transformer agent."""
        ...
        print(f"Initialized diffusion policy transformer agent on device: {device}")
    ...
    def evaluate_agent(self, dataloader, epoch: int) -> Dict[str, float]:
        ...
        pbar = tqdm(dataloader, desc=f"Evaluating Epoch {epoch}", leave=False)
    ...
    def train(
        ...
        """
        Main training loop for the diffusion policy transformer.
        ...
        for epoch in range(num_epochs):
            ...
            if epoch % test_every == 0:
                print(f"\nTesting transformer agent in environment (epoch {epoch})...")
                ...
                print(f"Success rate: {current_success_rate:.3f}")
                print(f"Best success rate: {self.best_success_rate:.3f}")
            
            # Save checkpoint
            if epoch % save_checkpoint_every == 0:
                checkpoint_file = checkpoint_path / f"transformer_checkpoint_epoch_{epoch}.pt"
            ...
            if test_metrics:
                print(f"Epoch {epoch}: Success Rate: {test_metrics['success_rate']:.3f}")
        
        # Save final model
        final_checkpoint = checkpoint_path / "final_transformer_model.pt"
        self.agent.save_model(final_checkpoint, save_optimizer=True, save_lr_scheduler=True)
        print(f"Training completed. Final transformer model saved: {final_checkpoint}")
