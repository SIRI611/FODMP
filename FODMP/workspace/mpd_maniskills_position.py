...
class MPDManiSkillsPosition(BaseWorkspace):
    """
    Movement Primitive Diffusion workspace for ManiSkills environments with position control.
    
    This workspace implements movement primitive diffusion for ManiSkills robotic manipulation tasks
    using position-based control. It extends the base workspace with MPD-specific training and 
    evaluation logic for ManiSkills environments.
    """
    ...
    def setup_agent(self, device: str = "cuda") -> None:
        """Initialize the movement primitive diffusion agent for ManiSkills position control."""
        ...
        print(f"Initialized MPD agent for ManiSkills position control on device: {device}")
    ...
    def setup_dataset(self, dataset_path: str) -> None:
        ...
        print(f"Loaded MPD ManiSkills dataset from: {dataset_path}")
    ...
    def train_epoch(self, dataloader, epoch: int) -> Dict[str, float]:
        ...
        epoch_losses = []
        pbar = tqdm(dataloader, desc=f"MPD ManiSkills Training Epoch {epoch}", leave=False)
    ...
    def evaluate_agent(self, dataloader, epoch: int) -> Dict[str, float]:
        ...
        pbar = tqdm(dataloader, desc=f"MPD ManiSkills Evaluating Epoch {epoch}", leave=False)
    ...
    def test_agent(self, agent: BaseAgent, num_trajectories: int = 10) -> dict:
        """Test the MPD agent in the ManiSkills environment."""
    ...
    def train(
        ...
        """
        Main training loop for movement primitive diffusion on ManiSkills.
        ...
        print(f"Starting MPD ManiSkills position training for {num_epochs} epochs...")
        ...
        for epoch in range(num_epochs):
            ...
            if epoch % test_every == 0:
                print(f"\nTesting MPD agent in ManiSkills environment (epoch {epoch})...")
                ...
                print(f"MPD ManiSkills Success rate: {current_success_rate:.3f}")
                print(f"Best success rate: {self.best_success_rate:.3f}")
            
            # Save checkpoint
            if epoch % save_checkpoint_every == 0:
                checkpoint_file = checkpoint_path / f"mpd_maniskills_position_checkpoint_epoch_{epoch}.pt"
                self.agent.save_model(checkpoint_file, save_optimizer=True, save_lr_scheduler=True)
                print(f"Saved MPD ManiSkills checkpoint: {checkpoint_file}")
            ...
            if test_metrics:
                print(f"Epoch {epoch}: MPD ManiSkills Success Rate: {test_metrics['success_rate']:.3f}")
        
        # Save final model
        final_checkpoint = checkpoint_path / "final_mpd_maniskills_position_model.pt"
        self.agent.save_model(final_checkpoint, save_optimizer=True, save_lr_scheduler=True)
        print(f"MPD ManiSkills training completed. Final position model saved: {final_checkpoint}")
    ...
    def load_checkpoint(self, checkpoint_path: str) -> None:
        ...
        print(f"Loaded MPD ManiSkills position checkpoint from: {checkpoint_path}")
