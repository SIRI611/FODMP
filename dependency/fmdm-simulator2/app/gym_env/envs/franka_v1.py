...
class FrankaEnv1(base_env.BaseGymEnv):
    """
    Franka gym environment for one arm, velocity action space, and observation space consisting of the rgb
    camera render data.
    """
    def __init__(self, config_file):
        ...
        assert self.render_mode == 'rgb_array', \
            'This environment\'s observation space consists of the rgb data so the render mode must be \'rgb_array\'.'
    ...
    def get_state(self):
        """
        Gets the rgb camera render data as a representation of the current state of the env.

        Returns
        -------
        tuple
            Two numpy arrays that correspond to the rgb camera data. RGB array has shape (height x width x 3) where the
            3 corresponds to the number of colors. In general terms it is an observation of the current state of the
            franka arm in the environment (same format as described by this envs observation_space).
        """
    ...
    def set_space_attributes(self):
        """
        Defines the action space, observation space, and reward range. The action space supports velocity control for a
        franka arm (9 dimension box with respective lower and upper bounds for each joint or end effector).
        Observation space consists of the rgb camera render data.
        """
        self.action_space = spaces.Box(
            low=-np.array(self.franka_robot.max_velocities),
            high=np.array(self.franka_robot.max_velocities),
            dtype=np.float32
        )
    ...
    def step(self, action):
        ...
        """
        self.franka_robot.apply_action_vel(action)
