...
class PlacementEnv0(base_env.BaseGymEnv):
    """
    Franka gym environment for one arm, positional action space, and observation space consisting of the rgb
    camera render data. Set up to perform the placement of a peg that starts in the robots hand into a slot
    """
    def __init__(self, config_file):
        ...
        self.plane_id, self.slot, self.peg, self.table_id = None, None, None, None
        self.table_height = 0
        ...
        self.init_d_xy, self.init_d_xyz = 1, 1
        self.max_steps = 80
        ...
        #self.set_space_attributes() # can't be called here since the robot is not yet initialized

    def reset_camera(self):
        """
        Resets the camera view and projection matrices for all cameras according to the values specified in the config file.
        """
        self.cameras = tuple( base_env.Camera(camera_config, self.bullet_client)
    ...
    def check_termination(self):
        ...
        """
        info = {'slot-collision': False, 'placement-limit-exceeded': False, 'successful-placement': False, 'dropped-peg': False, 'table-collision': False}
        if self.config_file['termination']['terminate-on-dropped-peg']['enabled']:
            self.holding_peg = self.franka_robot.is_grasping_object(self.peg)
            if not self.holding_peg:
                peg_pos, _ = self.bullet_client.getBasePositionAndOrientation(self.peg)
                if peg_pos[2] < self.table_height:
                    info['dropped-peg'] = True
                elif len(self.bullet_client.getContactPoints(self.peg,self.table_id)) > 0:
                    info['dropped-peg'] = True
   #             if self.dropped_peg_timer > self.config_file['termination']['terminate-on-dropped-peg']['dropped-peg-timeout']:
   #                 info['dropped-peg'] = True
        ...
        if self.config_file['termination']['terminate-after-successful-placements']['enable']:
            peg_pos, _ = self.bullet_client.getBasePositionAndOrientation(self.peg)
            slot_pos, _ = self.bullet_client.getBasePositionAndOrientation(self.slot)
            dist = np.sqrt(np.power(np.array(peg_pos)-np.array(slot_pos),2).sum())
            if dist < 0.04:
                contactPoints = self.bullet_client.getContactPoints(self.peg,self.slot)
                if contactPoints != []:
                    positionOnA, z = 5, 2
                    for contactPoint in contactPoints:
                        if contactPoint[positionOnA][z] - slot_pos[z] < 0.01:
    ...
    def get_reward(self, info):
        ...
        #     return 1
        # return -0.1
        return self.get_stage_reward(info)
    ...
    def initialize_scene(self):
        ...
        path_to_table_urdf = os.environ['path-to-assets'] + 'table/table.urdf'
        self.plane_id = scene_utils.load_plane(self.bullet_client, path_to_plane_urdf)
        self.table_id, self.table_height = scene_utils.load_table(self.bullet_client, path_to_table_urdf)
        ...
        self.peg, self.slot = placement_utils.load_peg_slot(
            self.bullet_client, 
    ...
    def calc_peg_slot_ref_pos(self, peg_bottom=0.07, slot_top=0.09, calll=''):
        ...
        slot_top_point = [slot_pos[0], slot_pos[1], slot_pos[2] + slot_top]
