...
class CollectAndClearEnv(base_env.BaseGymEnv):
    """
    ...
    def initialize_scene(self):
        ...
        self.container_id, self.container_info = collect_and_clear_utils.load_container(
            self.bullet_client,
    ...
    def get_stage_reward(self, info, epsilon1=0.004, epsilon2=-0.01, lambda_=2, c_a=0.5, c_b=0.25):
        ...
        if d_xy > epsilon1:  # r_reach
            r_reach = 1 - 0.5 * (
                        math.tanh(lambda_ * d_xyz / self.init_d_xyz) + math.tanh(lambda_ * d_xy / self.init_d_xy))
    ...
    def get_image_obs(self, patch_size_half=20):
        cam_images = self.render(self.render_mode)  # 0: 128 * 128*3, 1 640*480*3
        obj_bbbox = []
        obj_patch_images=[]
        for obj in self.target_objects + self.disruptive_objects:
            ...
            x_border_left, x_border_right = image_utils.crop_left_right_to_border(imv_xy[0], patch_size_half, cam_images[1].shape[0])
            y_border_top, y_border_down = image_utils.crop_left_right_to_border(imv_xy[1], patch_size_half, cam_images[1].shape[1])
            # print("{},{},{},{}".format(x_border_left, x_border_right, y_border_top, y_border_down))
            obj_patch_images.append(np.array(cam_images[1])[y_border_top:y_border_down,x_border_left:x_border_right,:])

        return obj_patch_images, obj_bbbox, np.array(cam_images[0])
    ...
    def apply_discrete_skills(self, skill_index, max_sim_steps=50, grasping_z=0.5904, place_down_z=1.0):
        # 0 move, do nothing
        ...
        if skill_index == 1:
            if not self.franka_robot.gripper_is_open:
                self.franka_robot.open_gripper()
            self.franka_robot.go_to_target_ee_pos([x, y, grasping_z], sim_steps=50)
            self.franka_robot.close_gripper()
            self.franka_robot.go_to_target_z_orn_top_down(z, sim_steps=20)
            # check if object picked up.
        elif skill_index == 2:
            self.franka_robot.go_to_target_ee_pos([x, y, place_down_z], sim_steps=20)
            self.franka_robot.go_to_target_ee_pos([x, y, place_down_z], sim_steps=20)  # place_down_z-0.2. put sim_steps = 50 when demo!!!!!!!!!!
            collect_and_clear_utils.reset_object(self.bullet_client, self.obj_id_to_remove, [100,100,0.1])
            self.franka_robot.open_gripper()
            self.franka_robot.go_to_target_ee_pos([x, y, z], sim_steps=20)
    ...
    def pick_up_object(self, obj_info, grasping_z=0.5804, dist_thres=0.005, max_approach_steps=20):
    ...
    def place_down_to_container(self, place_down_z=1.05, dist_thres=0.04, max_approach_steps=20):
        # return action: dx, dy, dz, discrete_skill_index
    ...
    def check_object_on_the_fly(self, obj_id, height_thres=0.70):
