...
class PutMugEnv(base_env.BaseGymEnv):
    """
    ...
    def initialize_scene(self):
        ...
        self.target_objects, self.disruptive_objects = collect_and_clear_utils.load_target_and_disruptive_objects(
            ...
            self.config_file['scene']['disruptive-objects'], target_color=[0.2,0.274,0.388,1], disruptive_color=[0.43921569, 0.38823529, 0.25098039, 1])
    ...
    def get_state(self):
        ...
        observation = {
            'state': continuous_states,  # np.expand_dims(robot_pos, axis=0).astype(np.float32),  dim 34
            'obj_imgs': obj_img_patches,  # np.expand_dims(img, axis=0)
            'top_view_img': top_down_view_img  # 128*128*3
        }
    ...
    def apply_discrete_skills(self, skill_index, max_sim_steps=50, grasping_z=0.5804, place_down_z=0.70):
        # 0 move, do nothing
        ...
        if skill_index == 1:
            ...
            self.franka_robot.go_to_target_ee_pos([x, y, place_down_z-0.1], sim_steps=500)  # place_down_z-0.2. put sim_steps = 50 when demo!!!!!!!!!!
            self.franka_robot.open_gripper()
            self.franka_robot.go_to_target_ee_pos([x, y, place_down_z], sim_steps=100)
    ...
    def pick_up_object(self, obj_info, grasping_z=0.5504, dist_thres=0.005, max_approach_steps=20):
    ...
    def place_down_to_container(self, place_down_z=0.80, dist_thres=0.04, max_approach_steps=20):
        # return action: dx, dy, dz, discrete_skill_index
