...
class PlaceCupEnv(base_env.BaseGymEnv):
    """
    ...
    def __init__(self, config_file):
        ...
        # metaworld_obj_utils.BASE=[ 1,-0.5,0.08 ]
    ...
    def check_success(self, thres=0.16):
        ...
        pos_cup, _ = self.bullet_client.getBasePositionAndOrientation(self.target_objects[0]['obj_id'])
        dist_linear = np.linalg.norm(np.array(pos_cup[0:2]) - np.array(pos_drawer[0:2]))
        dist_z = pos_cup[2]-pos_drawer[2]
        # print('dist linear {}, dist z {}'.format(dist_linear, dist_z))
        if dist_linear<=0.08 and dist_z>0.15:
            return  True
        else:
            return  False
    ...
    def get_state(self):
        ...
        # print(top_down_view_img.shape)  # (224,224,3)
        # print(corner1_img.shape)
        # print(corner2_img.shape)
    ...
    def apply_discrete_skills(self, skill_index, max_sim_steps=50, grasping_z=0.005, place_down_z=0.35):
        # 0 move, do nothing
    ...
    def pick_up_object(self, obj_info, grasping_z=0.005, dist_thres=0.005, max_approach_steps=40):
    ...
    def place_down_to_container(self, place_down_z=0.35, dist_thres=0.04, max_approach_steps=30):
        # return action: dx, dy, dz, discrete_skill_index
    ...
    def expert_action_one_robot(self):
        ...
        if obj_status == 3: # successfully approached the container
            discrete_skill_index = 2  # placedown skill
            self.target_objects[i]['status'] = 10
            self.obj_id_to_remove = self.target_objects[i]['obj_id']
            # self.current_target_obj_index += 1  # always,, 1 object suport
            return [0, 0, 0, discrete_skill_index]
        return [0,0,0,0]
