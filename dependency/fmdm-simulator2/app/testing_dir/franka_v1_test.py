...
@pytest.mark.franka_v1
class TestFranka1Env:
    def test_set_space_attributes(self, env_instance, config):
        """
        Tests the set_space_attributes method of Franka1Env. Ensures that the current env instance is an instance of
        Franka1Env and if so, asserts that the number of arms in the env are correct and that the observation and action
        space as well as the reward range, all have the correct shape.

        Parameters
        ----------
        env_instance : gym_envs.envs.franka_v1.Franka1Env
            An instance of a franka gym env.
        config : dict 
            File consisting of simulation config.
        """
        if not isinstance(env_instance, Franka1Env):
            pytest.skip('Working environment is not an instance of Franka1Env')
    ...
    def test_reset(self, env_instance, config):
        """
        Tests the reset method of Franka1Env. Ensures that all the space attributes are set properly and that the
    ...
    def test_step(self, env_instance, config):
        """
        Tests the step method of Franka1Env. Ensures that the current env instance is an instance of Franka1Env and if
