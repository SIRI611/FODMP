import os
from gym_env.envs.placement_v0 import PlacementEnv0

DEFAULT_POSITION = [0, 0, 0.4+0.625]
...
def setup():
    ...
    """
    x, y, z = 0, 1, 2
    env = PlacementEnv0(config_file=os.environ['path-to-configs'] + 'interactive_placement.yaml')
...
if __name__ == '__main__':
    """
        ...
        """
    camera_step_interval = 10
    camera_step_counter = 0
    env, target_x, target_y, target_z, orn_0, orn_1, orn_2, orn_3 = setup()
    camera_select = env.bullet_client.addUserDebugParameter("Select Camera View", 0, len(env.cameras) - 1,
                                                            len(env.cameras) - 1)
    enable_gui(env.bullet_client)
    pos = [target_x, target_y, target_z]
    orn = [orn_0, orn_1, orn_2, orn_3]
    ee_open, done = False, False
    print_helper_info()
    while not done:
        # done, info = env.check_termination()
        res = check_key_events(env, pos, orn, ee_open)
        camera_step_counter += 1
        # if not camera_step_counter % camera_step_interval:
        #     selected_camera = round(env.bullet_client.readUserDebugParameter(camera_select))
        #     env.cameras[selected_camera].render(env.sim_mode, env.render_mode)
        #     camera_step_counter = 0
        if len(res) > 0:
            if 'toggle ee' in res:
                ee_open = not ee_open
